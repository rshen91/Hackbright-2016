    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    for n in numbers:
        ...
        if len(numbers) < 0: #empty list 
            ...
            smallest.append(n)
            next = next_number
        elif next_number < next:
            smallest.append(next_number)
            smallest.pop(next)
    return smallest
...
def halvesies(numbers):
    ...
    lst = []
    for n in numbers:
        half = float(n)/2 #have to add float
        lst.append(half)
    return lst
...
def reverse_list(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    You should do this without changing the original list::
        ...
        ['apple', 'berry', 'cherry']
    """
    return items[::-1]#stride backwards to the most left
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    """
    index_lst = []  
    for item in words: #iterate through each word in the list
        for letters in words: #iterate through each letter in each word
            if letters == letter: 
                index_lst.append(int(item[letters])) #add the index to the empty list
    return index_lst
